...
import com.barclays.card.Dijkstra_Algorithm;
import com.barclays.card.Edge;
import com.barclays.card.Graph;
import com.barclays.card.Vertex;
...
public class Test extends TestCase {
	...
	private Graph graph;
	private Dijkstra_Algorithm dijkstra;
	private Map<String, Vertex[]> bags;
	private Map<String, Vertex> departures;

	public void testExcute() throws PathNotFoundException {
		nodes = new ArrayList<Vertex>();
		edges = new ArrayList<Edge>();
		logger.info("Vertexes and Edges are created");
		// Add Vertexes
		for (int i = 0; i <= 11; i++) {
			if (i == 0) {
	...
	private void getPathDistance(Map.Entry<String, Vertex[]> bag) throws PathNotFoundException {
		logger.debug("getting distance of the path ");
	...
	private void addLane(String id, int sourceLoc, int destLoc, int duration) {
		logger.debug(" Bydirection edges getting source location ");
		// Bidirectional Edges
		Edge lane = new Edge(id, nodes.get(sourceLoc), nodes.get(destLoc), duration);
		edges.add(lane);
		logger.debug(" Bydirection edges getting destination location ");
